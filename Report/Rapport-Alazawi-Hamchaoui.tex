...
\author{Al Azawi Rayan - Hamchaoui Ilyas}
\date{\today}
...
À ce jour, la plupart des objectifs du projet ont été realisés, dont :
\begin{itemize}
  \setlength\itemsep{1em}
  \item Objectif principal : Développer un whiteboard interactif doté d'une intelligence artificielle permettant la reconnaissance des intentions utilisateur et la génération automatique de formes géométriques.
  \item Objectif intermédiaire : Permettre la jonction automatique entre des formes Anchorable et des tracés Linkable (lier différentes formes avec un tracé).
  \item L'intégration entre les différents composants du système.
\end{itemize}
\begin{figure}
    \centering
    \includegraphics[width=1.1\linewidth]{Diagramme vierge(1).jpeg}
    \caption{schéma d'organisation temporelle}
    \centering
